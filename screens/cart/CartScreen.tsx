...
import CartItemCard from "./parts/CartItemCard";
...
export default function CartScreen({
  ...
  const handlePressCheckout = () => {
    ...
    setOrder(order);
    navigation.navigate(ROUTES_NAME.CHECKOUT);
  };

  const handleSetNote = (note: string, idItem: string) => {
    const item = cartItems.find(
      (cartItem: ICartItem) => cartItem.id === idItem,
    );
    const idxItem = cartItems.findIndex(
      (cartItem: ICartItem) => cartItem.id === idItem,
    );
      ...
      {cartItems.length === 0 ? (
        ...
        <Box flex='1'>
          <HStack
            ...
            alignItems='center'>
            <Checkbox
              isChecked={selectedItemsForOrder.length === cartItems.length}
              onChange={(e) => handleChangeValue(e)}>
              <Text fontSize='sm'>Pilih semua</Text>
            </Checkbox>
            <Pressable>
              <Text fontSize='md' fontWeight='bold' color='tertiary.500'>
                Hapus
              </Text>
            </Pressable>
          </HStack>
          ...
          <VStack
            ...
            <HStack
              ...
              <VStack space='1'>
                ...
                <Text fontSize='lg' lineHeight='xs' fontWeight='bold'>{`Rp ${
                  cartItemsTotalPayment * daysOfRent
                }`}</Text>
              </VStack>
              <Button
                onPress={handlePressCheckout}
                isDisabled={itemsCount <= 0 || daysOfRent === 0}
                variant='solid'

...
export default function DetailItemScreen({ navigation, route }: RootStackScreenProps<'DetailItem'>) {
    ...
    return (
        <Box backgroundColor='blueGray.200'>
            <ScrollView>
                <VStack>
                    <Image w='full' style={{ aspectRatio: 1, resizeMode: 'cover' }} src={detailItem.image ? detailItem.image : ''} alt='item-picture' />
                    <Box px='4' pt='4' pb='6' backgroundColor='white'>
                        <HStack justifyContent='space-between' alignItems='center'>
                            <Text fontSize='2xl' fontWeight='bold'>{`Rp ${detailItem.price}`}</Text>
                        ...
                        <HStack space='5' alignItems='center'>
                            <Text fontSize='xs'>{`Terjual ${detailItem.rented}`}</Text>
                            <Text fontSize='xs'>{`Tersisa ${detailItem.stock}`}</Text>
                            <HStack space='2' borderWidth='1' py='0.5' px='1' rounded='md' borderColor='gray.200'>
                                <Ionicons
                                    name="star"
                                    size={15}
                                    color='#fde047'
                        ...
                        <Box mt='4' backgroundColor='white'>
                            <Text fontSize='md' fontWeight='bold'>Deskripsi Produk</Text>
                            <Text fontSize='sm' >{detailItem.description || '-'}</Text>
                        </Box>
                    </Box>
                </VStack>
                <Box mt='4' p='4' backgroundColor='white'>
                    <Text fontSize='md' fontWeight='bold'>Ulasan Produk</Text>
                    <ReviewBox />
                    <ReviewBox />
                </Box>
            </ScrollView>
            <Box w='full' shadow='9' roundedTop='md' p='2' h='16' backgroundColor='white' position='absolute' bottom='0'>
                <HStack space='3' alignItems='center'>
                    <Button onPress={handlePressBuyNow} flexGrow='1' variant='outline' borderColor='tertiary.500' _text={{ color: 'tertiary.500', fontWeight: 'bold' }}>Beli Langung</Button>
                    <Pressable onPress={handlePressAddItemToCart} flexGrow='1' backgroundColor='tertiary.500' p='2.5' rounded='sm'>
                        <HStack space='1' alignItems='center' justifyContent='center'>
                            <FontAwesome
                                ...
                                size={10}
                            />
                            <Text fontWeight='bold' color='#fff'>Keranjang</Text>
                        </HStack>
                    </Pressable>
                </HStack>
            </Box>

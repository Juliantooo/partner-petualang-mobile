...
export const cartItemsSlice = createSlice({
  ...
  reducers: {
    ...
    REMOVE_CART_ITEM: (state, action: PayloadAction<string>) => {
      const newWishListItems = filterDataById(state.cartItems, action.payload);
      return {
        ...state,
    ...
    SUBTRACT_CART_ITEM: (state, action: PayloadAction<string>) => {
      const item: ICartItem = state.cartItems.find(
    ...
    SET_CART_ITEM_COUNT: (state, action: PayloadAction<ISetCartItemCount>) => {
      const item: ICartItem = state.cartItems.find(

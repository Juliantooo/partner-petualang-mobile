...
export const updateWishlistUser = async (
  wishlistItems: Array<IWishlistItem>,
  id: string,
) => {
  const userRef = doc(db, "users", id);

  const response = await updateDoc(userRef, { wishlistItems }).then(
    async () => {
      const userData = await getUserData(id);
      return userData;
    },
...
export const updateCartItemsUser = async (
...
export const updateOrdersHistoryUser = async (
  ordersHistory: Array<ICartItem>,
  id: string,
) => {
  const userRef = doc(db, "users", id);
  const response = await updateDoc(userRef, { ordersHistory }).then(
    async () => {
      const userData = await getUserData(id);
      return userData;
    },

...
const useOrderItems = () => {
  ...
  const orderItems = useSelector(
    (state: RootState) => state.orderItems.orderItems,
  );
  const ordersHistory = useSelector(
    (state: RootState) => state.orderItems.ordersHistory,
  );
  ...
  const setOrderItems = ({
    ...
    if (isShowNotification) {
      notification.success(
        "Lanjutkan pembayaran untuk menyelesaikan transaksi",
      );
    }
  };
  ...
  useEffect(() => {
    if (startRentDate && endRentDate) {
      ...
      const start = dayjs()
        .set("date", parseInt(startDate[0]))
        .set("month", parseInt(startDate[1]))
        .set("year", parseInt(startDate[2]));
      const end = dayjs()
        .set("date", parseInt(endDate[0]))
        .set("month", parseInt(endDate[1]))
        .set("year", parseInt(endDate[2]));

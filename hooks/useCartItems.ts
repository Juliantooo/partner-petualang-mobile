...
const useCartItems = () => {
  ...
  const findItemWithId = (id: string) =>
  ...
  const addCartItemCount = (id: string) => {
    ...
    if (item.count === item.stock)
      return notification.error("Jumlah barang melebihi stock barang");
  ...
  const subtractCartItemCount = (id: string) => {
    const item: ICartItem = findItemWithId(id)!;
    if (item.count === 1)
      return notification.error("Minimal jumlah peminjaman 1 barang");
    dispatch(SUBTRACT_CART_ITEM(id));
  };

  const calculateTotalPayment = async () => {
    const discount = selectedItemsForOrder.reduce(
      (result: number, item: ICartItem) =>
        result + item.price! * item.count! * (item.discount! / 100),
      0,
    );
    const totalPayment = selectedItemsForOrder.reduce(
      (result: number, item: ICartItem) => result + item.price! * item.count!,
      0,
    );
  ...
  const calculateItemsCount = () => {
    const count = selectedItemsForOrder.reduce(
  ...
  const removeItemFromSelectedItemForOrder = (id: string) => {
    const items: IOrderItem[] = filterDataById(selectedItemsForOrder, id);
  ...
  useEffect(() => {
    ...
    if (selectedItemsForOrder.length > 0) {
      ...
      const newSelectedItemsForOrder = cartItems.filter((item: ICartItem) =>
